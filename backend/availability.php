...
function publicAvailability(): array {
    ...
    $rows = query('SELECT s.*, EXISTS(SELECT 1 FROM ab_bookings b WHERE b.status IN (\'pending\',\'confirmed\') AND b.starts_at < s.ends_at AND b.ends_at > s.starts_at) AS reserved FROM ab_slots s WHERE s.active=1 AND s.starts_at >= ? AND s.starts_at < ? ORDER BY s.starts_at LIMIT 500',
    ...
    $slots = array_map(function ($row) use ($blocked, $now) {
        $dto=slotDTO($row);
        $dto['status']=in_array($dto['date'],$blocked,true) ? 'blocked' : (local($row['starts_at']) <= $now ? 'past' : ($row['reserved'] ? 'booked' : 'available'));
        return $dto;
    },$rows);
